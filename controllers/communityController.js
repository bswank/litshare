...
exports.nearCommunitiesPage = async (req, res) => {
  ...
  // Wes's Code:
  // const coordinates = [req.user.lng, req.user.lat].map(parseFloat);
...
// exports.nearCommunities = async (req, res) => {
//   const coordinates = [req.user.lng, req.user.lat].map(parseFloat);
//   const q = {
//     location: {
//       $near: {
...
//           coordinates
//         },
//         $maxDistance: 1000000 // 10000m or 10km
//       }
//     }
//   }
//
//   const communities = await Community.find(q).select('name slug');
//   res.json(communities);
// };
...
// We can get any document that's referenced by another document:
//
// const communities = await Community.find({
//   _id: { $in: req.user.field }
// });
//
// For instance: get the communities that are in the user field. So, if 'field' = 'isIn' then any community a user IS IN can be listed.
